  ...
  return <section id="skills" className="py-24 relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillCategories.map((category, index) => <div key={index} className="glass p-6 rounded-2xl card-shadow animate-slide-up" style={{
          animationDelay: `${index * 100}ms`
              ...
              <div className="space-y-3">
                {category.skills.map((skill, skillIndex) => <div key={skillIndex} className="bg-muted px-4 py-2 rounded-lg text-sm hover:bg-primary/20 transition-colors duration-300">
                    {skill}
                  </div>)}
              </div>
            </div>)}
